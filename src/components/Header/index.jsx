...
const Header = () => {
  ...
  return (
    <section className={`w-screen fixed top-0 left-0 z-[200] transform transition-transform duration-300 ${
        hidden ? "-translate-y-full" : "translate-y-0"
      }`} id='header'>
        <div className='w-full h-fit flex justify-between px-[4vw] pt-[2vw]'>
            <Link href={"/"} className='w-[3vw] h-[3vw]'>
                <Image src={"/assets/icons/enigma-logo.svg"} alt='enigma-logo' width={100} height={100} className={`w-full h-full object-contain ${isInverted ? " brightness-[16]" : ""}`}/>
            </Link>
            <div className='w-fit  flex gap-[1vw] text-white'>
                <Link href={"/"} className='bg-primary w-fit h-fit flex justify-center items-center rounded-full px-[2vw] py-[1vw] leading-[1] '>
                    Let's Talk
                </Link>
                <div className='w-fit flex'>
                    <Link href={"/"} className='bg-black-1 px-[2vw] py-[1vw] leading-[1] w-fit h-fit text-white rounded-full'>
                        Menu
